...
jdouble splice(JNIEnv *env, jclass obj,jstring img1,jstring img2,jstring img3){
   ...
   // LOGLN("Finding features, time: " << ((getTickCount() - t) / getTickFrequency()) << " sec");
    ...
    //LOGD("Warping images, time: " << ((getTickCount() - t) / getTickFrequency()) << " sec");
    ...
    if (!timelapse)
    {
       ...
       // imshow("result", result);
        imwrite(CUtil::jstringTostring(env,img3), result,compression_params);
    }
...
jdouble merge(JNIEnv *env, jclass obj,jstring img1,jstring img2,jstring img3,jstring img4,jstring path){
    //img1 img2 输入图片的路径    img3 合成后图片路径
    double time = getTickCount();
  ...
  //  LOGD("Warping images, time=%f" ,((getTickCount() - t) / getTickFrequency()),);
    ...
    Proto_frame1 = imread(CUtil::jstringTostring(env,img1));
    Proto_frame2 = imread(CUtil::jstringTostring(env,img2));

    for (size_t step_turn = 0; step_turn < 250; step_turn++)
    {
        //t = getTickCount();
        Mat img_warped, img_warped_s;
        Mat dilated_mask, seam_mask, mask, mask_warped;
       ...
       // imgleft  = "/storage/emulated/0/DCIM/Camera/image/left/"  + step_turn + ".jpg";
        //imgright = "/storage/emulated/0/DCIM/Camera/image/right/" + step_turn + ".jpg";
        ...
        for (int img_idx = 0; img_idx < num_images; ++img_idx)
        {
            ...
            else
            {
                ...
                {
                case 0:
                    full_img = imread(imgleft);;
                    break;
                case 1:
                    full_img = imread(imgright);
                    break;
                }
            }
            // Read image and resize it if necessary
        ...
        if (!timelapse)
        {
           ...
           // LOGD("Compositing, time=%f: " , ((getTickCount() - t) / getTickFrequency()));

            //imwrite(result_name, result);
            //imgname = step_turn + ".jpg";
            //cout << imgname << endl;
    ...
    LOGD("Finished, total time=%f: ", ((getTickCount() - app_start_time) / getTickFrequency()));
